...
function argsToDict(args: string[]) {
  let result = {};
...
async function readFileAsync(fname: string): Promise<Buffer> {
...
async function main(args: string[]) {
  ...
  let handler: RequestListener = (req, res) => {
    ...
    if (stat.isDirectory()) {
      ...
      for (let fname of fnames) {
        ...
        ssr.create("span", `file-${escapedFname}`, "file")
        .textContent(fname)
        .attrs({
          "onclick": "fnav(this);"
        })
  ...
  let port = 3000;
  if (dict.port) try { let n = parseInt(dict.port); port = n; } catch (ex) { console.warn("Malformed port", dict.port, "defaulting to", port); }
